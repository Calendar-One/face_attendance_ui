...
import { Camera, Upload, Check, X, User, RefreshCw } from 'lucide-react';
...
const FaceRegister: React.FC = () => {
  ...
  const captureImage = (): void => {
    if (capturedImages.length >= 10) {
      setMessage({ text: 'Maximum number of images (10) reached', type: 'error' });
      return;
    }
    ...
    if (video && canvas) {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          setMessage({ text: 'Image captured successfully', type: 'success' });
        }
      }, 'image/jpeg', 0.8);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 bg-gray-50 min-h-screen">
      <h1 className="text-2xl font-bold text-center mb-6">Face Registration System</h1>
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <div className="flex justify-between mb-4">
            <button 
              ...
              disabled={isLoading || !stremLoaded || capturedImages.length >= 10}
          ...
          <div className="flex justify-between">
            <button
              ...
              disabled={isLoading || capturedImages.length === 0 || !userId.trim()}
        ...
        <div>
          ...
          {verificationResult && (
            <div className="mb-4 p-4 bg-white rounded-lg shadow">
              ...
              <div className={`p-3 rounded ${verificationResult.verified ? 'bg-green-100' : 'bg-red-100'}`}>
                <p><span className="font-medium">Status:</span> {verificationResult.verified ? 'Verified ✓' : 'Not Verified ✗'}</p>
                {verificationResult.verified && verificationResult.user_id && verificationResult.confidence_score && (
                  <>
                    <p><span className="font-medium">User ID:</span> {verificationResult.user_id}</p>
                    <p><span className="font-medium">Confidence:</span> {Math.round(verificationResult.confidence_score * 100)}%</p>
                  </>
                )}
              </div>
            </div>
